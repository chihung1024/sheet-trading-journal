...
from journal_engine.config import API_KEY, EXCHANGE_SYMBOL
...
def _get_user_holding_symbols(user_df: pd.DataFrame) -> set[str]:
    """Compute current holdings symbols from BUY/SELL net qty (> 0).

    NOTE:
    - Uses transaction history to infer net position size.
    - This is used only to decide which symbols to fetch atomic realtime quotes for.
    """
    if user_df is None or user_df.empty:
        return set()

    df = user_df[user_df['Type'].isin(['BUY', 'SELL'])].copy()
    if df.empty:
        return set()

    df['signed_qty'] = df.apply(lambda r: r['Qty'] if r['Type'] == 'BUY' else -r['Qty'], axis=1)
    net = df.groupby('Symbol')['signed_qty'].sum()
    holdings = set(net[net.abs() > 1e-6].index.astype(str).str.upper().tolist())
    return holdings
...
def main():
    ...
    logger.info("=== 啟動交易日誌更新程序 (v2.58 per-user atomic realtime) ===")
    ...
    # FX is required for EOD series
    if EXCHANGE_SYMBOL:
        all_tickers.add(EXCHANGE_SYMBOL)
    ...
    logger.info(f"開始下載全域市場數據（EOD only）。標的數: {len(all_tickers)}")
    market_client.download_data(list(all_tickers), fetch_start_date, enable_realtime=False)
    ...
    for user_email in user_list:
        try:
            ...
            label = f"user:{user_email}"
            with market_client.atomic_realtime_context(sorted(list(required_symbols)), logger=logger, label=label) as ok:
