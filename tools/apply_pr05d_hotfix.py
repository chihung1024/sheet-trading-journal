...
start = text.index("async function handleGitHubTrigger(")
end = text.index("\nfunction validateCorsRequest(", start)
replacement = r'''async function handleGitHubTrigger(request, env, ctx, principal, requestId) {
  try {
    if (!env.GITHUB_TOKEN) {
...
manifest = Path("worker-manifest.json")
manifest.write_text(
    manifest.read_text()
    .replace('"releaseVersion": "4.05"', '"releaseVersion": "4.05.1"')
    .replace('"apiVersion": "2.57"', '"apiVersion": "2.58"')
)

wrangler = Path("wrangler.toml")
wrangler.write_text(
    wrangler.read_text()
    .replace('RELEASE_VERSION = "4.05"', 'RELEASE_VERSION = "4.05.1"')
    .replace('API_VERSION = "2.57"', 'API_VERSION = "2.58"')
)

deploy = Path(".github/workflows/deploy-worker.yml")
deploy.write_text(
    deploy.read_text().replace(
        'version.release_version !== "4.05" || version.api_version !== "2.57"',
        'version.release_version !== "4.05.1" || version.api_version !== "2.58"',
    )
)
