  ...
  <div class="app-layout" :class="{ 'dark-mode': isDark }">
    ...
    <div v-else class="main-wrapper">
      ...
      <div v-if="showGroupSwitchModal" class="modal-overlay" @click.self="showGroupSwitchModal=false">
        <div class="modal-card">
          <h3>切換策略群組</h3>
          <p class="modal-desc">請選擇要檢視的投資組合：</p>
          ...
          <div class="group-switch-list">
            ...
            >
              <span class="g-name">{{ g === 'all' ? '全部 (All Portfolios)' : g }}</span>
              <span v-if="portfolioStore.currentGroup === g" class="g-check">✓</span>
            </button>
          </div>
          
          <div class="modal-footer">
            <button class="btn-secondary" @click="showGroupModal=true; showGroupSwitchModal=false">管理群組</button>
...
const showGroupSwitchModal = ref(false); // ✨ 新增：控制切換群組視窗
...
.modal-footer .btn-secondary { background: var(--bg-secondary); color: var(--text-sub); border: none; }
...
.mobile-group-btn:hover { background: var(--bg-card); border-color: var(--primary); }
...
.group-switch-item:hover { background: var(--bg-card); border-color: var(--primary); }
...
.g-check { color: var(--primary); font-weight: 700; }
...
@media (max-width: 480px) {
  ...
  /* 顯示手機版切換按鈕 */
  .mobile-group-btn { display: flex; }
  
  /* 更新按鈕文字隱藏，僅顯示 Icon */
  .action-trigger-btn { padding: 8px; }
  .action-trigger-btn .btn-text { display: none; }
}
</style>
