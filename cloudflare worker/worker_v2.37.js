...
async function handleGitHubTrigger(req, env, user) {
  ...
  try {
    ...
    console.log('[DEBUG] No request body or parse error:', e.message);
  }
  
  console.log('[DEBUG] customBenchmark:', customBenchmark);
  console.log('[DEBUG] user.email:', user.email);
  
  const ghUrl = `https://api.github.com/repos/${env.GITHUB_OWNER}/${env.GITHUB_REPO}/actions/workflows/update.yml/dispatches`;
  console.log('[DEBUG] GitHub API URL:', ghUrl);
  ...
  console.log('[DEBUG] Request body to GitHub:', JSON.stringify(requestBody));
  ...
  console.log('[DEBUG] GitHub API response status:', resp.status);
  
  if (!resp.ok) {
    const errorText = await resp.text();
    console.error(`[GitHub API Error] Status: ${resp.status}, Body: ${errorText}`);
    return jsonResponse({ 
        success: false, 
