...
const TEST_OUTPUT = resolve('.wrangler/pr10d2-staging-render-test.toml');
...
test('staging renderer produces the exact isolated Worker configuration and dry-runs', async () => {
  ...
  try {
    ...
    assert.match(config, /^main = "\.\.\/staging-worker\.js"$/m);
    ...
    assert.equal(resolve(dirname(TEST_OUTPUT), '../staging-worker.js'), resolve('staging-worker.js'));
