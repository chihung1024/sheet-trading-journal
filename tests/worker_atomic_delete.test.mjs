...
test("missing record remains a definite 404 without unguarded snapshot cleanup", async () => {
  ...
  const response = await __test.handleDeleteRecord(request, { DB }, USER, "req-missing");
...
test("last-record delete reports RELOAD_UI from the same atomic batch", async () => {
  ...
  const response = await __test.handleDeleteRecord(request, { DB }, USER, "req-last");
  const body = await response.json();
...
test("non-last record delete preserves normal deleted response", async () => {
  ...
  const response = await __test.handleDeleteRecord(request, { DB }, USER, "req-non-last");
  const body = await response.json();
...
test("atomic batch failure fails closed as a database error", async () => {
  ...
  const response = await __test.handleDeleteRecord(request, { DB }, USER, "req-failure");
  const body = await response.json();
