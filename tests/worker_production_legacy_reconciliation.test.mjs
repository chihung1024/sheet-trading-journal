...
test("reconciliation starts from reviewed protected-main request and retains production reviewer gate", () => {
  ...
  assert.match(workflow, /Verify exact activation authority before reviewer gate/);
  assert.match(workflow, /verify_production_activation_authority\.mjs/);

  const requestStepStart = workflow.indexOf("Validate exact protected-main reconciliation request");
  const authorityStepStart = workflow.indexOf("Verify exact activation authority before reviewer gate");
  assert.ok(requestStepStart >= 0 && authorityStepStart > requestStepStart);
  const requestStep = workflow.slice(requestStepStart, authorityStepStart);
  assert.doesNotMatch(requestStep, /steps\.request\.outputs/);
});
...
test("production mutation uses latest protected-main control-plane tool while retaining exact runtime workspace", () => {
  assert.match(
    workflow,
    /control_plane="\$RUNNER_TEMP\/e1c-a1-reconciliation-control-plane"[\s\S]*git worktree add --detach "\$control_plane" refs\/remotes\/origin\/main/,
  );
  assert.match(
    workflow,
    /WRANGLER_CONFIG: \$\{\{ github\.workspace \}\}\/\.wrangler\/deploy\.toml/,
  );
  assert.match(
    workflow,
    /node "\$control_plane\/tools\/reconcile_legacy_calculation_jobs\.mjs" execute/,
  );
  assert.doesNotMatch(
    workflow,
    /run:\s+node tools\/reconcile_legacy_calculation_jobs\.mjs execute/,
  );
});
