...
test('dividend recovery copy preserves same-key reconciliation and distinguishes commit from readback confirmation', async () => {
  const source = await readSource('../src/components/DividendManager.vue');

  assert.match(source, /const outcome = await store\.addRecord\(record, \{ returnOutcome: true \}\)/);
  ...
  assert.match(source, /配息已保存；重新計算狀態將由系統持續追蹤與恢復，無需重複操作。/);
