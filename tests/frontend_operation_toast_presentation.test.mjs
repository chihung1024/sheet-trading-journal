...
const technicalTerms = /快照|後端計算工作|計算工作|原計算工作|原交易識別碼|原識別碼|相同識別碼|觸發重算|數據重算|背景計算|計算服務|投資組合計算|後端重算|更新數據|計算已完成/;
...
const cases = [
  ...
  ['⚠️ 計算已完成；最新資料暫時載入失敗，系統將自動重試', '⚠️ 資料更新已完成；最新資料暫時載入失敗，系統將自動重試'],
  ...
  ['後端重算觸發失敗，請稍後手動觸發「更新數據」', '資料更新失敗，請稍後使用「立即更新」再試'],
  ['觸發重算失敗：服務暫時無法使用', '資料更新失敗：服務暫時無法使用'],
