...
test("public version endpoint exposes source and runtime traceability without authentication", async () => {
  ...
  assert.equal(body.release_version, "4.11");
  assert.equal(body.api_version, "2.64");
  ...
  assert.equal(response.headers.get("X-Release-Version"), "4.11");
  assert.equal(response.headers.get("X-API-Version"), "2.64");
...
test("build metadata sanitizes untrusted deployment variables", () => {
  ...
  assert.equal(metadata.release_version, "4.11");
  assert.equal(metadata.api_version, "2.64");
