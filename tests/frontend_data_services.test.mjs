...
test('logout cleanup removes every dynamic pending-calculation generation but preserves neighboring keys', () => {
  const generationA = `${PENDING_CALCULATION_V2_STORAGE_PREFIX}1800000000000.abcdefghijklmnop`;
  const generationB = `${PENDING_CALCULATION_V2_STORAGE_PREFIX}1800000000100.qrstuvwxyzABCDEF`;
  const storage = storageWith({
    token: 'secret',
