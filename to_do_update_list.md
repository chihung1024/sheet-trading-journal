...
- There is no active feature roadmap, staging program, CI program, audit/evidence program or deployment-control project.
...
- Final terminal release branch is created after the cleanup PR is merged and must point to the resulting `main` SHA.

**Validation evidence**
- Last full terminal validation baseline: `00becc3157d5be65076a6adadd50fc8e0b4fb93b`.
- GitHub Actions CI run #1406 passed Python runtime tests, Worker runtime tests, Worker production contract verification, local D1 migration/schema verification, frontend runtime tests and the production frontend build.
- The following structural strip removes only the development validation harness and does not change `src/`, Worker runtime modules, D1 migrations, calculation runtime or `update.yml`.
...
- D1 rebuild authority: retained migrations `0001` through `0006`.
...
- [x] Create pre-cleanup recovery branch.
...
- [x] Preserve current broker-neutral import and Journal Restore after reachability proved they are live product features.
- [x] Complete full runtime/build/schema validation before removing the validation harness.
- [x] Remove tests, CI workflow and test-only development dependencies after the last green validation baseline.
